...
def execute(dn_rgb_img: Path, dn_rgb_txt: Path, dn_calib: Path):
    ...
    dist_params = np.zeros((4, 1))
    rmse, camera_matrix, dist_params, rvecs, tvecs \
        = cv2.fisheye.calibrate(objpoints, imgpoints, (rgb_img.shape[1], rgb_img.shape[0]),
    ...
    # getOptimalNewCameraMatrix() is not implemented for fisheye cameras,
    # so manually create a new optimal camera matrix
    new_camera_matrix = camera_matrix.copy()
    new_camera_matrix[0, 0] *= 0.6
    new_camera_matrix[1, 1] *= 0.6
    remap = cv2.fisheye.initUndistortRectifyMap(camera_matrix, dist_params, np.eye(3), new_camera_matrix,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Calibration script for a fisheye camera")
